...
# Internal imports
from app.optimizer.problem import FullFP, FullFPVariable
from app.optimizer.utils import postProcessing, IO
...
class Optimizer:
    ...
    def execute(self):
        ...
        if self.selected_function == 'Fixed mean':
            problem = FullFP(self.N_VAR, self.N_OBJ,
                             self.N_CONSTR, self.jsondata)

        elif self.selected_function == 'Variable mean':
            problem = FullFPVariable(self.N_VAR, self.N_OBJ,
                                     self.N_CONSTR, self.jsondata)
        ...
        if self.selected_method['algorithm'] == 'NSGA2':
            ...
            ref_dirs = get_reference_directions("das-dennis", 2,
                                                n_partitions=self.selected_method['parameters'][1])

            algorithm = NSGA3(pop_size=self.selected_method['parameters'][0],
                              ref_dirs=ref_dirs)
            ...
            algorithm = MOEAD(ref_dirs,
                              n_neighbors=self.selected_method['parameters'][0],
                              decomposition="pbi",
                              prob_neighbor_mating=0.7, seed=1)

        elif self.selected_method['algorithm'] == 'C-TAEA':
            ref_dirs = get_reference_directions("das-dennis", 2,
                                                n_partitions=self.selected_method['parameters'])

            algorithm = CTAEA(ref_dirs, seed=1)

        result = minimize(problem, algorithm,
                          ('n_gen', self.N_GEN), verbose=True, seed=8)

        result_int = postProcessing.floatToInteger(result)
        IO.saveCSV(result_int, self.outputpath)
